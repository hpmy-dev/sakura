...
)
{
	...
	// スレッド数 = clamp(max(設定値, 論理コア数 / 4), 4, 16)  ※設定値はiniファイルの nGrepThreadCount（4〜16）
	const int nIniThreads = GetDllShareData().m_Common.m_sSearch.m_nGrepThreadCount;
	const auto nClampedIni = (unsigned int)std::max( 4, std::min( nIniThreads, 16 ) );
	const unsigned int nThreads = std::min<unsigned int>(
		std::max<unsigned int>( nClampedIni, std::thread::hardware_concurrency() / 4 ),
		16 );
