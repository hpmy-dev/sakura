...
/*!
 * @brief 文字コード名の同時取得 (IRR-47)
 * @remark 複数スレッドから CCodeTypeName::Bracket() を同時に呼んでも、
 *		   戻り値が "  [<短縮名>]" の形式を保つことを確認する。
 *		   組み立て用バッファがプロセス共有だと、並列 Grep のワーカー間で
 *		   内容が混ざり "  [EUC]EUC]" のような文字列が Grep 結果に出力される。
 * @note   競合は確率的に顕在化するため反復回数を多めに取る。合格は「競合が無いこと」の
 *		   証明にはならないが、共有バッファへ戻してしまった際の回帰検出には働く。
 */
TEST(Irregular, CCodeTypeName_BracketFromMultipleThreads_NotCorrupted) {
	...
	EXPECT_EQ(0, nMismatch.load()) << "Bracket() の戻り値が壊れました:" << samples;
}
